...
__all__ = ["integrate_sde", "SDEIntegrator"]
...
def _get_sde_step_function(method: str) -> Callable:
    if callable(method):
        sde_step = method
    elif method == 'Euler-Maruyama:su(n)':
        sde_step = special_unitary_euler_maruyama_algebra_step
    else:
        raise ValueError(f"Unsupported integration method: {method}")
    return sde_step


def special_unitary_euler_maruyama_algebra_step(
    func: Callable,
    t: float,
    y: torch.Tensor,
    dt: float,
    ...
    *args
    ...
    """
    Perform a single Euler–Maruyama integration step for SU(n) matrices.
    ...
    *args :
    ...
    scale = abs(dt)**0.5 * noise_scale
    diffusion = scale * randn_traceless_antihermitian_like(y)
