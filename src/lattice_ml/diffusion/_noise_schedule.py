...
class VPInverseTimeNoiseSchedule(torch.nn.Module):
    ...
    def sigma_square(self, t:torch.Tensor) -> torch.Tensor:
        """Compute the instantaneous noise variance at time `t`.
    ...
    def gamma(self, t: torch.Tensor) -> torch.Tensor:
        """Compute the gamma function associated to the scheduler.
...
# =============================================================================
class SubVPInverseTimeNoiseSchedule(torch.nn.Module):
    """
    Noise standard deviation scheduler derived from an inverse-time variance
    law: Var(t) = 2 t / (1 - t) in a Sub Variance-Preserving (SubVP) setting.
    ...
    def forward(self, t: torch.Tensor) -> torch.Tensor:
        ...
        Returns:
            torch.Tensor: Standard deviation of noise at time `t`.
        """
        return (2 * self.gamma(t) * self.cumulative_noise_scale(0, t)) ** 0.5
    ...
    def sigma_square(self, t:torch.Tensor) -> torch.Tensor:
        ...
        Args:
            t (torch.Tensor): Time tensor with values in (0, 1).
        """
        return 2 * self.gamma(t) * self.cumulative_noise_scale(0, t)
