...
class GaugeLinkConv(torch.nn.Module):
    ...
    def __init__(
        ...
        self.weight_poly = torch.nn.Parameter(torch.randn(*shape) * 0.01)
    
    def time_dependent_weight(self, t: torch.Tensor | float) -> torch.Tensor:
        """
        Returns weights evaluated at time t.
        If t is scalar -> shape: (ndim, 2*(ndim-1), C_out, C_in, 2)
        If t is (B,)   -> shape: (B, ndim, 2*(ndim-1), C_out, C_in, 2)
        """
        # Shapes:
    ...
    def forward(self, x: torch.Tensor, t: torch.Tensor | float = 0.0) -> torch.Tensor:
        ...
        """
        # Ensure complex dtype so all buffers (#staples) are complex
        if not torch.is_complex(x):
            x = x.to(torch.complex64 if x.dtype in (torch.float16, torch.bfloat16, torch.float32)
