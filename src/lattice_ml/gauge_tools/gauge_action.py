...
from .wilson_loops import compute_planar_wilson_1x1_loop, compute_planar_string_1x1_loop
from .wilson_staples import compute_staples


__all__ = ['WilsonGaugeAction, StringGaugeAction']
...
class StringGaugeAction:
    r"""
    String gauge action and force calculations for lattice gauge theory.

    Implements the String gauge action for SU(N_c) gauge group, together with
    the corresponding gauge force used in HMC simulations.
    ...
    Here:
        - :math:`\beta` is the inverse coupling.
        - :math:`\text{Plaq}_{\mu\nu}` is the plaquette in the (mu, nu) plane.
        - :math:`\text{Plaq}(x)_{\mu\nu} = 
        V^\dagger_\mu(x)V_\mu(x+\mu)V^\dagger_\nu(x+\mu)V_\nu(x+\mu+\nu)
        V^\dagger_\mu(x+\mu+\nu)V_\mu(x+\nu)V^\dagger_\nu(x+\nu)V_\nu(x)`
    ...
    def __call__(self, x: torch.Tensor) -> torch.Tensor:
        ...
        for mu in range(1, spatial_ndim):
            for nu in range(mu):
                plaq = compute_normalized_trace(compute_planar_string_1x1_loop(
                    x, mu, nu, sites_before_link=self.sites_before_link
                )).real
                plaq_sum += torch.sum(plaq, dim=sum_dims)
    ...
    def algebra_force(self, x: torch.Tensor) -> torch.Tensor:
        ...
        g = compute_staples(
            x, sites_before_link=self.sites_before_link, sum_over_staples=True, is_string = True,
        )
